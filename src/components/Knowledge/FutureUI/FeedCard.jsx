...
const FeedCard = ({ item, isFollowing, onToggleFollow, onVote, isFavorited, onToggleFavorite }) => {
  ...
  return (
    <div className={`bg-[#0f0f0f] border rounded-2xl p-5 transition-all group relative shadow-[0_2px_16px_rgba(0,0,0,0.25)] ${
      isAd
        ? 'border-[rgba(200,160,30,0.15)] bg-[rgba(200,160,30,0.03)]'
        : 'border-white/[0.06] hover:bg-[rgba(200,30,30,0.03)] hover:border-[rgba(200,30,30,0.16)] hover:shadow-[0_4px_24px_rgba(0,0,0,0.35)]'
    }`}>
      ...
      <div className="flex items-center gap-2 mb-2.5 flex-wrap">
        {!isAd && (
          <div className="flex items-center gap-1.5">
            ...
            {item.author?.role && (
              <span className="text-[10px] text-white/25 truncate max-w-[100px] font-[Hanken_Grotesk,sans-serif]">{item.author.role}</span>
            )}
          </div>
        )}
      ...
      <Link href={`/knowledge/question/${item.id}`}>
        <h2 className="text-sm md:text-[15px] font-semibold text-white/90 leading-snug mb-2 group-hover:text-[#c81e1e] transition-colors flex items-start gap-2 font-[Hanken_Grotesk,sans-serif]">
          <span className={`shrink-0 font-[JetBrains_Mono,monospace] text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded-full mt-0.5 border ${
            item.type === 'question'
              ? 'text-[#c81e1e] border-[rgba(200,30,30,0.28)] bg-[rgba(200,30,30,0.08)]'
      ...
      <div className="flex gap-3 mb-3">
        {item.hasImage && (
          <div className="w-20 h-14 shrink-0 rounded-xl overflow-hidden border border-white/[0.06] bg-white/[0.04]">
            <img src={item.imageUrl} className="w-full h-full object-cover" alt="content" />
          </div>
        )}
        <div className="flex-1 min-w-0">
          <p className="text-[12px] text-white/35 leading-relaxed line-clamp-2 md:line-clamp-3 font-[Hanken_Grotesk,sans-serif] font-[300]">
      ...
      <div className="flex items-center justify-between mt-1 pt-2.5 border-t border-white/[0.04]">
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-1 bg-[rgba(200,30,30,0.08)] text-[#c81e1e] px-2.5 py-1 rounded-full text-[11px] font-[JetBrains_Mono,monospace] border border-[rgba(200,30,30,0.2)] hover:bg-[rgba(200,30,30,0.16)] transition-colors"
          >
            ...
            <button
              ...
              className={`flex items-center gap-0.5 font-[JetBrains_Mono,monospace] text-[9px] uppercase tracking-wider px-2 py-0.5 rounded-full border transition-colors ${
                isFollowing?.(item.topic.id)
                  ? 'bg-[rgba(200,30,30,0.1)] text-[#c81e1e] border-[rgba(200,30,30,0.2)] hover:bg-[rgba(200,30,30,0.2)]'
      ...
      {!isAd && item.hot_comment && (
        <div className="mt-3 bg-white/[0.02] border border-white/[0.06] p-2.5 rounded-xl flex items-center gap-2 text-[10px]">
          <span className="text-[#c81e1e] font-[JetBrains_Mono,monospace] uppercase tracking-wider flex items-center gap-1 shrink-0">
            <Flame className="w-2.5 h-2.5" /> Hot
          </span>
          <span className="text-white/30 truncate font-[Hanken_Grotesk,sans-serif]">{item.hot_comment}</span>
        </div>
      )}
    </div>
  );
