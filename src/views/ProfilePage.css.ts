...
export const saveBtn = style({ display: 'inline-flex', alignItems: 'center', padding: `${vars.space.xs} ${vars.space.lg}`, background: vars.color.success, color: vars.color.text, borderRadius: vars.radius.md, border: 'none', fontSize: '13px', fontWeight: 500, fontFamily: vars.font.body, cursor: 'pointer', transition: 'background 160ms ease', selectors: { '&:hover': { background: '#2d8a4e' }, '&:disabled': { opacity: 0.5 } } });
...
export const certNumber = style({ fontFamily: vars.font.label, fontSize: '11px', color: vars.color.textFaint });
